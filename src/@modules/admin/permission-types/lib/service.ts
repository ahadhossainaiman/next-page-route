import { IBaseFilter, IBaseResponse, TId } from '@base/interfaces';
import { AxiosInstanceSecret } from '@lib/config';
import { responseHandlerFn, Toolbox } from '@lib/utils';
import { IPermissionType, IPermissionTypeCreate, IPermissionTypeUpdate } from './interfaces';

const END_POINT: string = '/permission-types';

export const PermissionTypesService = {
  NAME: END_POINT,
  async find(options: IBaseFilter): Promise<IBaseResponse<IPermissionType[]>> {
    try {
      const res = await AxiosInstanceSecret.get(`${END_POINT}?${Toolbox.queryNormalizer(options)}`);
      return Promise.resolve(res?.data);
    } catch (error) {
      throw responseHandlerFn(error);
    }
  },
  async findById(id: string): Promise<IBaseResponse<IPermissionType>> {
    try {
      if (!id) return null;
      const res = await AxiosInstanceSecret.get(`${END_POINT}/${id}`);
      return Promise.resolve(res?.data);
    } catch (error) {
      throw responseHandlerFn(error);
    }
  },
  async create(payload: IPermissionTypeCreate): Promise<IBaseResponse<IPermissionType>> {
    try {
      const res = await AxiosInstanceSecret.post(END_POINT, payload);
      return Promise.resolve(res?.data);
    } catch (error) {
      throw responseHandlerFn(error);
    }
  },
  async update(payload: IPermissionTypeUpdate): Promise<IBaseResponse<IPermissionType>> {
    try {
      const res = await AxiosInstanceSecret.patch(`${END_POINT}/${payload.id}`, payload.data);
      return Promise.resolve(res?.data);
    } catch (error) {
      throw responseHandlerFn(error);
    }
  },
  async delete(id: TId): Promise<IBaseResponse<IPermissionType>> {
    try {
      const res = await AxiosInstanceSecret.delete(`${END_POINT}/${id}`);
      return Promise.resolve(res?.data);
    } catch (error) {
      throw responseHandlerFn(error);
    }
  },
};
